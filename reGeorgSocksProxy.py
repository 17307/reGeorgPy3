...
class session(Thread):
    ...
    def parseSocks5(self, sock):
        ...
        if atyp == b"\x01":  # IPv4
            ...
            for i in range(len(target) / 2):
                tmp_addr.append(chr(ord(target[2 * i]) * 256 + ord(target[2 * i + 1])))
        ...
        if cmd == b"\x02":  # BIND
            ...
            try:
                ...
                log.error("oeps")
            serverIp = bytes([int(i) for i in serverIp.split(".")])
            ...
            if self.cookie:
                ...
                raise RemoteConnectionFailed(
                    "[%s:%d] Remote failed" % (target, targetPort)
                )
    ...
    def parseSocks4(self, sock):
        ...
        if cmd == b"\x01":  # Connect
            ...
            if self.cookie:
                sock.sendall(
                    b"\x00" + b"\x5A" + struct.pack(">h", targetPort) + serverIp
                )
                return True
            else:
                sock.sendall(
                    b"\x00" + b"\x91" + struct.pack(">h", targetPort) + serverIp
                )
                raise RemoteConnectionFailed("Remote connection failed")
        else:
            raise SocksProtocolNotImplemented(
                "Socks4 - Command [%d] Not implemented" % ord(cmd)
            )
    ...
    def setupRemoteSession(self, target, port):
        ...
        response = conn.urlopen(
            "POST",
            self.connectString + "?cmd=connect&target=%s&port=%d" % (target, port),
            headers=headers,
            body="",
        )
        if response.status == 200:
            ...
            if status == "OK":
                ...
                log.info(
                    "[%s:%d] HTTP [200]: cookie [%s]" % (self.target, self.port, cookie)
                )
            else:
                if response.headers.get("X-ERROR") is not None:
                    log.error(response.headers.get("X-ERROR"))
        else:
            log.error(
                "[%s:%d] HTTP [%d]: [%s]"
                % (
            ...
            log.error(
                "[%s:%d] RemoteError: %s" % (self.target, self.port, response.data)
            )
        conn.close()
        return cookie
    ...
    def closeRemoteSession(self):
        ...
        response = conn.request(
            "POST", self.httpPath + "?cmd=disconnect", params, headers
        )
        if response.status == 200:
    ...
    def reader(self):
        ...
        while True:
            try:
                ...
                response = conn.urlopen(
                    "POST", self.connectString + "?cmd=read", headers=headers, body=""
                )
                data = None
                if response.status == 200:
                    status = response.headers.get("x-status")
                    if status == "OK":
                        ...
                        try:
                            if (
                                response.headers.get("server").find("Apache-Coyote/1.1")
                                > 0
                            ):
                        ...
                        data = None
                        log.error(
                            "[%s:%d] HTTP [%d]: Status: [%s]: Message [%s] Shutting down"
                            % (
                                self.target,
                                self.port,
                                response.status,
                ...
                if len(data) == 0:
                    ...
                    continue
                transferLog.info(
                    "[%s:%d] <<<< [%d]" % (self.target, self.port, len(data))
                )
    ...
    def writer(self):
        ...
        while True:
            try:
                ...
                data = self.pSocket.recv(READBUFSIZE).decode()
                if not data:
                    break
                headers = {
                    "X-CMD": "FORWARD",
                ...
                response = conn.urlopen(
                    "POST",
                    self.connectString + "?cmd=forward",
                    headers=headers,
                    body=data,
                )
                if response.status == 200:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Socks server for reGeorg HTTP(s) tunneller"
    )
    parser.add_argument(
        "-l",
        "--listen-on",
    ...
    parser.add_argument(
        ...
        "--read-buff",
        metavar="",
        help="Local read buffer, max data to be sent per POST",
        type=int,
        default="1024",
    )
    parser.add_argument(
        ...
        metavar="",
        required=True,
        help="The url containing the tunnel script",
    )
    parser.add_argument(
        "-v", "--verbose", metavar="", help="Verbose output[INFO|DEBUG]", default="INFO"
    )
    ...
    log.info(
        "Starting socks server [%s:%d], tunnel at [%s]"
